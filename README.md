...
Dự án này là một hệ thống Image Retrieval hiện đại, sử dụng mô hình học sâu **CLIP** (của OpenAI) để trích xuất đặc trưng và **ChromaDB** để lưu trữ/tìm kiếm vector siêu tốc. 
...
4. (Tùy chọn) Chạy script test logic cũ trên console:
   ```bash
   python src/main.py
   ```
